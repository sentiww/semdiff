...
def wup_similarity(a: str, b: str, *, logger: logging.Logger = LOGGER) -> float | None:
    synset_pair = resolve_synset_pair(
        a, b, logger=logger, metric_name="WUP similarity"
    )
    if synset_pair is None:
        return None

    a_synset, b_synset = synset_pair
    similarity = a_synset.wup_similarity(b_synset)
