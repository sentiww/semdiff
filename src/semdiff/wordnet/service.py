...
from semdiff.core.protocols import ISemanticMetric
...
class WordNetMetric(ISemanticMetric):
    name: str
...
class WordNetService:
    ...
    def create_metric(self, name: str) -> ISemanticMetric:
        ...
        return WordNetMetric(name=name, _calculator=calculator)
    ...
    def _path_distance(self, a: str, b: str) -> int | None:
        ...
        if synset_pair is None:
            logger.warning("Path distance: synset pair resolution failed for %r and %r", a, b)
            return None
        left, right = synset_pair
        distance = left.shortest_path_distance(right, simulate_root=True)
        if distance is None:
            logger.warning("Path distance: no path found between %s and %s", left.name(), right.name())
    ...
    def _resolve_synset_pair(
        ...
        *,
        ...
        if left is None or right is None:
            logger.warning(
                "%s unavailable for x=%r y=%r (could not resolve synset)",
                metric_name,
                a,
                b,
            )
    ...
    def _parse_synset(self, value: str) -> Synset | None:
        ...
        if wnid_match:
            pos = wnid_match.group(1)
            offset = int(wnid_match.group(2))
            logger.debug("Attempting synset_from_pos_and_offset(%r, %s)", pos, offset)
            try:
                synset = wn.synset_from_pos_and_offset(pos, offset)
                logger.debug("Resolved %r to synset %s", value, synset.name())
                return synset
            except Exception as exc:
                logger.warning(
                    "synset_from_pos_and_offset(%r, %s) failed: %s. Trying wn.synset(%r).",
                    pos,
                    offset,
                    exc,
                    normalized,
        ...
        try:
            ...
            logger.debug("Resolved %r via wn.synset to %s", value, synset.name())
            return synset
        except (LookupError, WordNetError, KeyError, AttributeError, ValueError) as exc:
            logger.warning("Could not resolve synset name %r: %s", value, exc)
            return None
