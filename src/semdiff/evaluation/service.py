...
from semdiff.datasets.synset_dataset import (
    ImageTransform,
    SynsetImageFolder,
...
if TYPE_CHECKING:
    from semdiff.evaluation.runtime import EvaluationRuntime
            ...
            with torch.inference_mode():
                for batch_number, (images, targets) in enumerate(dataloader, start=1):
                    try:
                        ...
                        logger.warning(
                            "Batch %s inference failed: %s. Skipping batch.",
                            batch_number,
                            exc,
                        )
                        totals.sample_offset += len(targets)
                        continue
